		...
		<form id="editForm" name="editForm" method="post" action="../landChange/list">
			...
			<fieldset class="fieldset_common_style">
				<table style="width: 100%" align="left">
					...
					<tr>
						<td class="table_common_td_label_style">变更原因：</td>
						<td colspan="5" class="table_common_td_txt_style" >
			...
			<fieldset class="fieldset_common_style">
				...
				<table style="width: 100%" align="center">
					<tr>
						<td align="center">
							<a href="#" class="easyui-linkbutton" onclick="javascript:showUploadDialog('04', '${landRegChange.id}', '举证资料', 'imgPriviewOuter', 'imgPriviewInner');">
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		setTimeout(function(){
			  ...
			  $('#townCodeView').combobox('disable'); 
			  $('#countryCodeView').combobox('disable'); 
		...
		Public.ajaxPost('save', JSON.stringify(a), function(e) {
			...
			if (200 == e.status) {
				$.messager.alert('提示','操作成功。','info',function(){
					if(retFlag == '1'){
					    document.location.href = '${ctx}/landRegChange/list?year=${year}&companyCode=${companyCode}&beginDate=${beginDate}&endDate=${endDate}&status=${status}&page=${page}&pageSize=${pageSize}';
					}else if("${flag}" == "audit"){
					    retList();
					    //document.location.href = '${ctx}/landRegChange/listquery?year=${year}&companyCode=${companyCode}&beginDate=${beginDate}&endDate=${endDate}&status=${status}&page=${page}&pageSize=${pageSize}';
					} else {
					    document.location.href = '${ctx}/landRegChange/editInput';
					}
				});
			} else {
...
function retList(){
	var url = "${ctx}/landChange/listQuerySearch";
...
function check(){
	...
	if($("#changeReason").val()==""){
		$.messager.alert('警告','请输入本次备案的变更原因！','warning');
		return false;
	}
	return true;
}
